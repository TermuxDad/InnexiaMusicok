...
@Client.on_message(filters.command(["deezer", f"deezer@{U}"]) & (filters.chat(CHAT) | filters.private))
...
admincmds=["joinvc", "unmute", "mute", "leave", "clean", "vc", "pause", "resume", "stop", "skip", "radio", "stopradio", "replay", "restart", "volume", f"volume@{U}", f"joinvc@{U}", f"unmute@{U}", f"mute@{U}", f"leave@{U}", f"clean@{U}", f"vc@{U}", f"pause@{U}", f"resume@{U}", f"stop@{U}", f"skip@{U}", f"radio@{U}", f"stopradio@{U}", f"replay@{U}", f"restart@{U}"]
...
async def not_chat(_, m: Message):
    buttons = [
        [
            InlineKeyboardButton('⚠️ Deploy to Heroku', url='https://heroku.com/deploy?template=https://github.com/DarkCybers/innexiaMusic'),
            InlineKeyboardButton('Source Code 💥', url='https://github.com/DarkCybers/InnexiaMusic'),
        ],
        [
            InlineKeyboardButton('🤥 How to Make', url='https://t.me/SiderzBot/6'),
            InlineKeyboardButton('Help & Commands ❔', callback_data='help')       
        ]
        ]
    k=await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE](https://github.com/DarkCybers/InnexiaMusic) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
    await mp.delete(m)
